...
export default function PackagesList({ realmId, onEstimateUpdate }) {
  ...
  // Keep skeleton widths aligned to columns
  const COL = {
    pkg: "w-24",
    est: "w-24",
    date: "w-28",
            ...
            {loading ? (
              ...
              Array.from({ length: 10 }).map((_, i) => (
                <tr key={i} className="border-b">
                  <td className="px-3 py-2"><Skel w={COL.pkg} /></td>
                  <td className="px-3 py-2"><Skel w={COL.est} /></td>
                  <td className="px-3 py-2"><Skel w={COL.date} /></td>
                  <td className="px-3 py-2">
                    <Skel w={i % 2 ? "w-72" : COL.cust} />
                  </td>
                  <td className="px-3 py-2"><Skel w={COL.driver} /></td>
                  <td className="px-3 py-2"><Skel w={COL.items} /></td>
                  <td className="px-3 py-2 text-right"><Skel w={COL.actions} /></td>
                </tr>
              ))
            ) : rows.length === 0 ? (
              <tr>
                ...
                <tr key={p._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    {Array.isArray(p.lines)
                      ? p.lines.reduce((sum, it) => sum + (Number(it.quantity) || 0), 0)
                      : 0}
                  </td>

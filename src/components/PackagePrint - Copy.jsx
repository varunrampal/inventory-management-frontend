...
const fmtDate = (d) => {
    ...
    const dt = new Date(raw); return isNaN(dt) ? "—" : dt.toLocaleDateString();
};
    ...
    function Field({ label, children, className = "" }) {
        return (
            <div className={className} style={{ breakInside: "avoid" }}>
                ...
                <div className="mt-1 font-medium">{children ?? "—"}</div> </div>

        );
    }

    return (
        <div ref={ref} className="min-h-screen bg-white text-black" style={{ WebkitPrintColorAdjust: "exact", printColorAdjust: "exact" }} >
            <div className="mx-auto max-w-4xl p-6">
                {/* Header */}
                <header className="flex items-start justify-between gap-4 border-b pb-4">
                    <div className="flex items-center gap-4">
                        <CompanyLogo companyId={realmId} height="30mm" />
                        <div> <h1 className="text-xl font-bold">{company?.name}</h1> {company?.address && <p className="text-sm text-black">{company.address}</p>}
                            <p className="text-sm text-black"> {[company?.phone, company?.email, company?.website].filter(Boolean).join(" · ")} </p>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-2xl uppercase tracking-wider text-black font-bold">Package</div>
                        <div className="text-lg font-extrabold"> {pkg?.packageCode || pkg?.packageId || "—"} </div>
                        {pkg?.estimateId && (
                            <div className="text-sm text-black"> Estimate: <span className="font-medium">{pkg.docNumber}</span>
                            </div>

                        )}
                        {/* <div className="text-lg text-black"> */}
                        {/* Ship Date: <span className="font-medium">{fmtDate(pkg?.shipmentDate)}</span> */}
                        {/* Ship Date: <span className="font-medium">{pkg?.shipmentDate}</span> */}
                        {/* </div> */}
                    </div>
                </header>
                {/* Info grid */}
                <section className="mt-4 rounded-lg border p-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                        <Field label="Customer"> {pkg?.customerName || pkg?.snapshot?.customerName || "—"} </Field>
                        {/* <Field label="Ship Date">{fmtDate(pkg?.shipmentDate)}</Field> */}
                        <Field label="Ship Date">{pkg?.shipmentDate}</Field>
                        <Field label="Estimate">{pkg?.docNumber || "—"}</Field>
